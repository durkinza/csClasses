		...
		while(!file.eof()){
			if(number <= clock){
				...
				if(!file.eof()){
					...
					if(iss >> number){}else{cout << "problem adding number.";}
				}else{break;}
			}else{
				break;
			}
...
void swtichLines(){
	...
	if(cus->activity == 'C' || cus->activity == 'D'){
		addToShortest(true, cus->entryTime, cus->name, cus->activity, cus->transactions);
	}else{
		addToShortest(false, cus->entryTime, cus->name, cus->activity, cus->transactions);
	}
}
